 ...
 * それにしても、めっちゃデコメントのテストするよ<br>
 * o mapStyle, map:{ sea = mystic ; land = oneman } そのまま書いても大丈夫？<br>
 * o mapStyle, map:\{ sea \= mystic \; land \= oneman \} エスケープされてても大丈夫？<br>
 * o DBコメントで、ベタッと 'シングルクォーテーション' で囲うけど大丈夫？<br>
 * o DBコメントで、ベタッと &lt;span&gt;HTMLタグ&lt;/span&gt; で囲うけど大丈夫？<br>
 * o DBコメントで、ベタッと &amp;lt;HTMLエスケープ表現&amp;gt; で囲うけど大丈夫？ (もともとSchemaHTML的にダメみたい!?)<br>
 * _/_/_/_/_/_/_/_/_/_/<br>
 * <br>
 * ※同じようなこと、カラムコメントの方でも試している。<br>
 * <br>
 * ※それにしても MySQL さん、エスケープがバックスラッシュなので、<br>
 * create文の中では「バックスラッシュ+シングルクォーテーション」ってなるし、<br>
...
public abstract class BsMember extends AbstractEntity implements DomainEntity {
     ...
     * 会員ID: mapStyle, map:{ sea = mystic ; land = oneman } そのまま書くぞー
     ...
     * 会員名称: mapStyle, map:\{ sea \= mystic \; land \= oneman \} 記号をエスケープするぞー
     ...
     * 会員アカウント: ベタッと 'シングルクォーテーション' で囲うぞー
     ...
     * 正式会員日時: ベタッと &lt;span&gt;HTMLタグ&lt;/span&gt; で囲うぞー
     ...
     * 生年月日: ベタッと &amp;lt;HTMLエスケープ表現&amp;gt; で囲うぞー (もともとSchemaHTML的にダメみたい!?)
